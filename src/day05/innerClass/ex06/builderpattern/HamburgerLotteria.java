...
public class HamburgerLotteria {
    public static void main(String[] args) {
        ...
        Hamburger 치킨버거 = new Hamburger.BurgerBuilder("플랫", "쇠고기", "콜라").addCheese().addBacon().addLettuce().build();
...
// 치킨버거(토핑 : 치즈, 야채, 베이컨) + 콜라 세트 메뉴 만들어 주세요

/*
 기존 생성자 오버로딩에서 보였던 가독성 문제가 해결되고, 파라미터에 대해 해당하는 Setter메서드를
 호출하면서 유연하게 객체 생성이 가능해짐.
 문제점 발생
 1. 일관성 문제 (consistency)
 필수 매개변수가 선택적으로 셋팅될 수 있으므로 일관성 문제가 발생한다.
 필수 멤버 : 객체가 포기화 될 때, 반드시 설정이 되어야 하는 값
 객체가 유효하지 않은 형태로 생성, 햄버거 인스턴스를 다른곳에서 사용하게 되면 런타임 예외가 발생할 수 있다.
 객체를 생성하는 부분과 값을 설정하는 부분이 물리적으로 떨어져있어서 발생하는 문제점이다.
